...
class AVL
{
    ...
    TreeNode * insertSubtree(TreeNode * subtree, T elem)
    {
        ...
        bool left_taller = (height(subtree->left) > height(subtree->right)+1);
        bool right_taller = (height(subtree->right) > height(subtree->left)+1);
        if((left_taller) && (elem < subtree->left->data))
        {
            LogLow("LL Scenario\n");
        ...
        if((right_taller) && (elem > subtree->right->data))
        {
            LogLow("RR Scenario\n");
