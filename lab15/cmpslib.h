...
inline void Prompt (string prompt, T & val)
{
	...
	cout << prompt;
	cin >> val;
	while(cin.fail())  // if it fails... ie user input does not match the datatype
	{
		cin.clear();
...
/* 
ValueAsString  
these convert any datatype to printable string
and if unknown it should print something with a usefull diagnostic value

there is a template one and a couple for char* the oddball
*/

inline string ValueAsString(char * in)
 {
			ostringstream out;
        out <<   in ;
        return out.str();
...
inline string ValueAsString(char const* in)
 {
			ostringstream out;
        out <<   in ;
        return out.str();
	...
	template <typename T>
		inline string ValueAsString(T & in)
		{
			...
			int status=0;
			char * tmp;
			tmp = abi::__cxa_demangle( typeid(T).name(),0,0,&status);
			string name(tmp);
			free (tmp);
